<?php
	 ...
	 */
	include_once($_SERVER["DOCUMENT_ROOT"] . "/filemanager/config/config.php");
	include_once($_SERVER["DOCUMENT_ROOT"] . "/src/Init/functions.php");
	...
	class UploadHandler
	{
		...
		public function post($print_response = true)
		{
			...
			$file_name = $content_disposition_header ? rawurldecode(preg_replace('/(^[^"]+")|("$)/', '', $content_disposition_header
				)
			) : null;
			...
			if ($upload) {
				if (is_array($upload['tmp_name'])) {
					...
					foreach ($upload['tmp_name'] as $index => $value) {
						$files[] = $this->handle_file_upload($upload['tmp_name'][$index], $file_name ? $file_name : $upload['name'][$index], $size ? $size : $upload['size'][$index], $upload['type'][$index], isset($upload['error']) ? $upload['error'][$index] : null, $index, $content_range
						);
					}
				} else {
				...
				if ($thumbResult !== true) {
					...
					if (!$this->options['ftp'] && !new_thumbnails_creation($targetPath, $targetFile, $_FILES['files']['name'][0], $this->options['config']['current_path'], $this->options['config'])) {
						...
						if ($this->options['config']['image_resizing']) {
							...
							} else if ($this->options['config']['image_resizing_height'] == 0) // if height not set
							{
