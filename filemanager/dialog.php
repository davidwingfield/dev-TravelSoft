<?php
	...
	$_paths = explode("/", trim(rtrim($subdir, "/")));
	$rootDir = $config["current_path"] . $rfm_subfolder;
	foreach ($_paths as $directory) {
		$rootDir .= "/" . $directory;
	...
	if (isset($sourceType) && isset($sourceId)) {
		$params = array(
	...
	function connectDB(): mysqli
	{
		...
		$username = "travelsoft_root";
		$password = "i5S=Sj)*wMb;O#R%";
		$dbname = "travelsoft";
	...
	function disconnectDB(mysqli $conn = null): void
	{
		if (!is_null($conn)) {
			$conn->close();
		}
	...
	function format(array $image = []): array
	{
	...
	function productImages(int $sourceId = null): array
	{
		if (is_null($sourceId)) {
			return [];
		}
		...
		$result = $conn->query("
			SELECT
	                    ...
	                    PRODUCT_IMAGE.is_cover AS 'image_is_cover_image',
		    ...
		    WHERE		PRODUCT_IMAGE.product_id = $sourceId;
		"
		);
		...
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				$temp = format($row);
				$id = null;
				$name = (isset($temp["name"])) ? $temp["name"] : null;
				$extension = (isset($temp["extension"])) ? $temp["extension"] : null;
	...
	function unitImages(int $sourceId = null): array
	{
		if (is_null($sourceId)) {
			return [];
		}
	...
	function companyImages(int $sourceId = null): array
	{
		if (is_null($sourceId)) {
			return [];
		}
		...
		$result = $conn->query("
			SELECT
                ...
                WHERE			COMPANY_IMAGE.company_id = $sourceId;"
		);
		...
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				//echo "row<pre>" . var_export($row, 1) . "</pre>";
				$path = (isset($row["image_path"])) ? $row["image_path"] : null;
				$name = (isset($row["image_name"])) ? $row["image_name"] : null;
				$extension = (isset($row["image_extension"])) ? $row["image_extension"] : null;
				if ($path && $name && $extension) {
					$id = "$name.$extension";
					if ($id) {
						$images[$id] = format($row);
					}
				}
			}
		}
	...
	if (isset($_GET["source"])) {
		if (isset($_GET["source_id"])) {
			...
			//echo "<pre>" . var_export($_GET, 1) . "</pre>";
			switch ($_GET["source"]) {
				case "company":
					$myImages = companyImages($sourceId);
					//echo "<pre>" . var_export($myImages, 1) . "</pre>";
					break;
				case "product":
					$myImages = productImages($sourceId);
					break;
				case "unit":
					$myImages = unitImages($sourceId);
					break;
				default:
					exit(0);
			}
	...
	//$myImages = productImages(1);
	
	//echo "<pre>" . var_export($myImages, 1) . "</pre>";
	//echo "<pre>" . var_export($dir, 1) . "</pre>";
	//echo "$cur_dir_thumb<br>";
	//echo "$thumbs_path<br>";
	//echo "$parent<br>";
	//echo "<pre>" . var_export($get_params, 1) . "</pre>";
?>
<!DOCTYPE html>
<html>
	<head>
	...
	<body>
		...
		<div class="container-fluid">
									...
									<div class="span4 half">
										...
										<div id="multiple-selection" style="display:none;">
											<?php if ($config['multiple_selection']) { ?>
												...
												<button class="tip btn select-cover-btn" title="<?php echo trans('Select_All'); ?>">
													<i class="icon-check"></i>
												</button>
			...
			<div class="row-fluid ff-container">
				<div class="span12">
					<?php if(
					($ftp && !$ftp->isDir($config['ftp_base_folder'] . $config['upload_dir'] . $rfm_subfolder . $subdir)) || (!$ftp && @opendir($config['current_path'] . $rfm_subfolder . $subdir) === false)){ ?>
					<br/>
					...
					<ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
						<?php
							...
							$image = (isset($myImages[$file])) ? $myImages[$file] : null;
							$isCover = false;
							$coverClass = "";
							
							if (!is_null($image)) {
								$isCover = (isset($image["is_cover_image"]) && $image["is_cover_image"] === 1) ? true : false;
								if ($isCover) {
									//$coverClass = "cover-image";
								}
							}
						...
						?>
						<li class="ff-item-type-<?php echo $class_ext; ?> file <?php if (!$config['multiple_selection']) { ?>no-selector<?php } ?> <?php echo $coverClass; ?>" data-name="<?php echo $file; ?>" <?php if (($filter != '' && stripos($file, $filter) === false)) {
							echo ' style="display:none;"';
						} ?>><?php
								$file_prevent_rename = false;
								$file_prevent_delete = false;
							...
							<figure data-name="<?php echo $file ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-type="<?php if ($is_img) {
								...
								<?php } ?>
								<a href="javascript:void(0)" class="link" data-file="<?php echo $file; ?>" data-function="<?php echo $apply; ?>">
									<div class="img-precontainer">
									...
									<?php if ($is_icon_thumb) { ?>
										<div class="cover"></div>
									<?php } ?>
									<div class="box <?php echo $coverClass; ?>">
										<h4 class="<?php if ($config['ellipsis_title_after_first_row']) {
											echo "ellipsis";
										} ?>">
								...
								<input type="hidden" class="is-cover" value="<?php echo $file_array['date']; ?>"/>
								<input type="hidden" class="date" value="<?php echo $file_array['date']; ?>"/>
								<input type="hidden" class="size" value="<?php echo $file_array['size'] ?>"/>
								...
								<figcaption>
									<form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
										...
										<a title="<?php echo trans('Download') ?>" class="tip-right" href="javascript:void('')" <?php if ($config['download_files']) echo "onclick=\"$('#form" . $nu . "').submit();\"" ?>><i class="icon-download <?php if (!$config['download_files']) {
												echo 'icon-white';
											} ?>"></i></a>
										...
										<?php } else if (($is_video || $is_audio) && in_array($file_array['extension'], $config['jplayer_exts'])) { ?>
										...
										<?php } else if (in_array($file_array['extension'], $config['cad_exts'])) { ?>
											<a class="tip-right file-preview-btn" title="<?php echo trans('Preview') ?>" data-url="ajax_calls.php?action=cad_preview&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
											   href="javascript:void('');"><i class=" icon-eye-open"></i></a>
										<?php } else if ($config['preview_text_files'] && in_array($file_array['extension'], $config['previewable_text_file_exts'])) { ?>
											<a class="tip-right file-preview-btn" title="<?php echo trans('Preview') ?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename; ?>&file=<?php echo $rfm_subfolder . $subdir . $file; ?>"
											   href="javascript:void('');"><i class=" icon-eye-open"></i></a>
										<?php } else if ($config['googledoc_enabled'] && in_array($file_array['extension'], $config['googledoc_file_exts'])) { ?>
